...
estimate fits) -- directly, with fake numbers, rather than only through
a full ManagedLoRATrainerNode.build() run (which can't exercise the
"release something" branch at all in this sandbox: DeviceContext.
for_device() on a CPU tensor returns _NullDeviceContext, whose
memory_stats() is always None -- see this class's own
record_step_peak() docstring for why that's actually the *correct*
behavior to fall back to, not just a testing limitation, and
smoke_test_managed_trainer.py's own check_*_when_calibration_can_never_
resolve for the integration-level confirmation of that fallback).
"""
...
def check_a_decision_is_final_once_made():
    print("[record_step_peak() after a decision is a no-op -- doesn't re-calibrate or "
    ...
    controller.record_step_peak({"peak_reserved_mb": 99999.0})  # must be ignored
    check(controller.should_release("optimizer") is False, "a later, huge reading must not reopen the decision")
    print("    PASS")
...
def main():
    ...
    check_a_decision_is_final_once_made()
    print()
    print("=" * 60)
