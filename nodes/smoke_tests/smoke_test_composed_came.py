...
a real offload-to-CPU-and-reload-to-device round trip (a thing numpy has
no concept of at all). This script exercises exactly those, on whatever
real device is actually available.
...
What it checks, in order:
  ...
  3. Specifically: does training continue correctly (no crash, loss still
     sane) after a full offload -> reload round trip? This is the
     highest-risk untested path -- state tensors get their identity
     replaced (state[name] = t.to(dev)) during offload/reload, so a bug
     here would show up as a crash or silently-wrong training after
     resuming, not at the offload/reload call sites themselves.

Prints a clear PASS/FAIL summary. Does not touch core/, manager/, server/,
or the training pipeline in any way -- fully standalone.
"""
...
def main():
    ...
    # --- 1. Real toy linear regression, real autograd ---
    ...
    handle = node.build(params=[W], lr=0.05, device=device)["optimizer"]
    ...
    if losses[-1] >= losses[0] * 0.5:
        failures.append("Loss did not decrease meaningfully")
        print("    FAIL: expected substantial loss reduction")
    else:
        print("    PASS")

    # --- 2. Lifecycle methods on real device tensors ---
    ...
    if handle.lr != 0.02:
        failures.append("update_lr did not update handle.lr")
        print(f"    FAIL: update_lr -- handle.lr={handle.lr}, expected 0.02")
    else:
        print(f"    update_lr(0.02): handle.lr correctly = {handle.lr}")

    # --- 3. The highest-risk untested path: offload -> reload round trip ---
    ...
    if not all(d == "cpu" for d in post_offload_devices.values()):
        failures.append(f"offload_states_to_cpu did not move all state to CPU: {post_offload_devices}")
    ...
    if not all(d == expected_type for d in post_reload_devices.values()):
        failures.append(f"reload_states_to_device did not restore device correctly: {post_reload_devices}")
    ...
    # Does training actually still work correctly after the round trip?
    ...
    if any(torch.isnan(torch.tensor(l)) or torch.isinf(torch.tensor(l)) for l in resumed_losses):
        failures.append("NaN/Inf loss after offload/reload round trip")
        print(f"    FAIL: NaN/Inf appeared in post-reload training")
    elif resumed_losses[-1] > loss_before_resume * 2:
        failures.append(f"Loss got substantially worse after offload/reload round trip: "
    ...
    if not all_zero:
        failures.append("reset_states did not zero all state")
    ...
    handle.free_states()
    if handle.states != []:
        failures.append("free_states did not clear the states list")
